...
bc=[0.9	0.75	1.5	3.5	0.95	165	5	0.75	0.65	1	1.15	0.015	0.7	0.25	0.578 7	1500	0.8	20	7 1.5	10000	60	5];  %optimis

titles=['Capture efficiency (%)', 'Capital cost (%)', ...
"Threshold CO3 concentration (M)", "Membrane permeance (L/m^2/h/bar)[GO]", "CO3 Rejection (%)[GO]", 'Membrane cost ($/m^2)[GO]', "Membrane Lifetime (years)[GO]", ...
'Stack cost (%)', 'Single pass conversion (%)', 'Current density (mA/cm^2)', 'Price of Product (%)', 'Electricity cost ($/kWh)', 'Faradaic efficiency C2H4 (%)', 'Faradaic efficiency C2H5OH (%)',  'Cell Voltage (%)', 'Electrode area per cell (m^2)', ...
'H2 permeance (GPU)[HF]', 'C2H4 permeance (GPU)[HF]', 'Membrane cost ($/m^2)[HF]', 'Membrane lifetime (years)[HF]', 'Water flux (LMH)[Pervap]', 'Membrane selectivity [Pervap]', 'Membrane cost ($/m^2)[Pervap]', 'Membrane lifetime (years)[Pervap]'];
...
V = (6.4* num(10)+ 3.0943)* num(15);         % Full cell voltage in volts from shin et al. (BPM), operating in the linear range of the Tafel plot
...
stacktot = ceil(Elecarea/areaperst);      
...
CRREchemR = ((cath_gdl + anod_gdl)*0.05 + bpm/2.5+ (bpp)/7) *num(8)*1.5*1.18;
...
CRRpervap = FCILpervap/num(20) /1000000;
E_sep = (CUTpervap+CUTdis+CUTZHF)*3600/num(12)/tpa; %MJ/kg C2H4
