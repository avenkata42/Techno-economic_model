%% AEM calulcations
% bc=[0.8	1	2	2.8	0.9	200	3	1	0.35	0.5	1	0.02	0.5	0.2	0.683	3.5	1100	1.2	25	6 1.5	10000	70	3];  %bc
% bc=[0.75	1.15	2	2.1	0.6	200	1	1.1	0.2	0.3	0.85	0.08	0.34	0.13	1	1.5	1100	1.2	35	5.5 1	8500	75	1];  %pessi
bc=[0.9	0.75	1.5 3.5	0.95	165	5	0.75	0.65	1	1.15	0.015	0.27	0.07	0.578 7	1500	0.8	20	7 1.5	10000	60	5];  %optimis

titles=['Capture efficiency', 'Capital cost', "Threshold CO3 concentration(M)", "Membrane permeance(GO)", "CO3 Rejection(GO)", 'Membrane cost(GO)', "Membrane Lifetime (GO)", 'Stack cost', 'Single pass conversion', 'Current density', 'Price of Product', 'Electricity cost', 'Faradaic efficiency C2H4', 'Faradaic efficiency C2H5OH',  'Cell Voltage', 'Electrode area per cell', 'H2 permeance(HF)', 'C2H4 permeance(HF)', 'Membrane cost(HF)', 'Membrane lifetime(HF)', 'Water flux(Pervap)', 'Membrane selectivity(Pervap)', 'Membrane cost(Pervap)', 'Membrane lifetime(Pervap)'];

%% Variable initialization
ming=zeros(length(titles),5);
maxg=zeros(length(titles),5);
tiledlayout(ceil(length(titles)),4)
counter=1;
highval=zeros(21,1);
lowval=zeros(21,1);
bc1=bc;
prodrate= 2000000;       % tpa
...
tpa=prodrate;         % desired C2H4 in MMT/yr or TPA
tr=0.39;              % tax rate
...
% Calculation Prpbability Dist based on triangular dist formulas
...
% mul3 = 1.5;
mul3 = 1;
kohCCU= co22bcap/0.13*56*RR *mul3;                           % kg/s

waterCCU= kohCCU/56*1000/kohconc+2*18*co22bcap;             % kg/s
CCUarea= co22bcap*44/capturerate/1000*3600*24*365*SF;       % m^2

%% --------------------------------------------------------change [2] 
% change in the CCU cost 1.77x
% mul2 = 1.77;
mul2 = 1;
FCILCCU= 4668*(CCUarea^0.85)*num(2)*1.18*1.5 * mul2;
...
%% --------------------------------------------------------change [2] 
% change in the CCU energy requirements for the regenartion loop
% e1_comprser = 132; % kWh/t-CO2
% e2_slaker = 77; % kWh/t-CO2
% e3_peletrctr = 27; % kWh/t-CO2
% e4_calciner = 4.05;% GJ/t-CO2
% CUTCCU = CUTCCU + (e1_comprser+e2_slaker+e3_peletrctr+e4_calciner*277.78)*co22bcap*44/1000*86400*365*SF*num(12)/1000000; % 1 GJ to 277.778 kWh, t-CO2 per annum
...
%% --------------------------------------------------------change [5]
% V = (0.2508* log(num(10)*1000)+ 1.106);  % Full cell voltage in volts from shin et al. (AEM)

V = (6.4* num(10)+ 3.0943)* num(15);         % Full cell voltage in volts from shin et al. (BPM)
...
% part_coeff = [196.044, 91.1621, 83.5651965, 189.9209885, 273.4860889, 330.083, 246.897237];              % AEM w/ catholyte 100
...
CUTEchemR = power*num(12)*24*365*SF/1000000;  % million USD
...
% (1) distillation
%% --------------------------------------------------------change [8]
% the water in the ditillation i
water = H2Oin-H2Oreq; % BPM
% water = H2Oreq;       % AEM

flow_in = (water + etha)/60;                 % L/min, density =1 
...
% q_a_gas=O2/1.43+(max_co2*0.5*1000)*44/1.84;                    %m^3/h
% FCILPSA= 1989043*(q_a_gas/1000)^0.7;
% CRRPSA= FCILPSA* 0.05/7 /1000000;
% CUTPSA= FCILPSA*0.2/1000000;
...
%% --------------------------------------------------------change [6]
% KOH regeneration
CUTregen = 312 * 3* ((ethy/28+etha/46))*24*365*SF*num(12)/1000000;       % (kJ/mol or kW.s/mol) *(kmol/h)*h*yr
% CUTregen=0;

%% Energy requirements
E_CCU = CUTCCU/num(12)*3600/tpa; %MJ/kg C2H4
E_GO = CUTGO/num(12)*3600/tpa; %MJ/kg C2H4
E_EchemR = CUTEchemR/num(12)*3600/tpa; %MJ/kg C2H4
E_sep = (CUTpervap+CUTdis+CUTZHF+CUTPSA)*3600/num(12)/tpa; %MJ/kg C2H4
E_regen = CUTregen/num(12)*3600/tpa; %MJ/kg C2H4

%% total CAPEX & OPEX
CUTI = CUTCCU + CUTGO + CUTEchemR + CUTZHF + CUTdis + CUTpervap+CUTPSA + CUTregen;      % million USD
FCILbop = 0.35*(FCILEchemR/0.65);                                     % USD 
FCIL = FCILCCU + FCILGO + FCILEchemR + FCILZHF + FCILdis + FCILbop + FCILpervap + FCILPSA;   % USD     
CRR = CRRCCU + CRRGO + CRREchemR + CRRZHF + CRRpervap + CRRdis + CRRPSA;       % million USD: Running and replacement
...
for bcpc=0:0.005:200                                     
    ...
    ROROI = sum(Arr(4:totyrs,1))/(totyrs-cy-1) / (FCIL / 10 ^ 6) *100;
